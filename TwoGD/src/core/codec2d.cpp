...
uint8_t codec2d::DrawLine(V2* v_pPointA, V2* v_pPointB, COLOR* c_pColor, uint8_t i_PixelFlag, uint8_t i_PrioFlag)
{
	...
	if (v_Delta.f_Pos[Y] == 0)
	{
		...
		{
			return DrawHLine(v_pPointB, (uint32_t)v_Delta.f_Pos[X], c_pColor);
		}
		return DrawHLine(v_pPointA, (uint32_t)-v_Delta.f_Pos[X], c_pColor);
	...
	if (v_Delta.f_Pos[X] == 0)
	{
		if (v_Delta.f_Pos[Y] > 0)
		{
			return DrawVLine(v_pPointB, (uint32_t)v_Delta.f_Pos[Y], c_pColor);
		}
		return DrawVLine(v_pPointA, (uint32_t)-v_Delta.f_Pos[Y], c_pColor);
	}
...
uint8_t codec2d::DrawRect(V2* v_pPointA, V2* v_pPointB, COLOR* c_pColor, uint8_t i_PixelFlag, uint8_t i_PrioFlag)
{
	...
	for (uint32_t i_Index = 0; i_Index < v_pPointB->f_Pos[1] - v_pPointA->f_Pos[1]; i_Index++)
	{
		...
		DrawHLine(&v_Temp, (uint32_t)(v_pPointB->f_Pos[0] - v_pPointA->f_Pos[0]), c_pColor);
	}
	return GD_TASK_OKAY;
}
...
uint8_t codec2d::DrawVLine(V2* v_pPoint, uint32_t  i_Length, COLOR* c_pColor, uint8_t i_PixelFlag, uint8_t i_PrioFlag)
{
...
uint8_t codec2d::DrawCanvas(DWORD* d_pBuffer, V2* v_pPos, uint32_t  i_Pixel[2], uint8_t i_PixelFlag, uint8_t i_PrioFlag)
{
...
uint8_t codec2d::DrawVMap(VMAP* o_VecMap, V2* v_pAnchor, float f_Scale, uint8_t i_PixelFlag, uint8_t i_PrioFlag)
{
...
uint8_t codec2d::DrawChar(CHARMAP* o_VecMap, V2* v_pAnchor, COLOR* c_pColor, float f_Scale)
{
	V2 v_H, v_L, v_Point;
	for (int i_Y = (int)(o_VecMap->i_YMin * f_Scale); i_Y < (int)(o_VecMap->i_YMax * f_Scale); i_Y++)
	{
		...
		for (int i_Index = 0; i_Index < i_EdgeCount; i_Index += 2)
		{
			...
			DrawHLine(&v_Point, (uint32_t)(f_EdgesSorted[i_Index + 1] - f_EdgesSorted[i_Index]), c_pColor, PF_FONT);
		}
	}
