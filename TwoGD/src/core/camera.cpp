...
camera::camera(uint32_t i_Width, uint32_t i_Height)
{
	...
	V3 v_Angles = V3(0, 0, 0);
	Relate(&v_ScreenPos, &v_Angles);
	f_CutoffAngles[0] = v_Angles.f_Pos[X] + DEGTORAD(15);
	f_CutoffAngles[1] = v_Angles.f_Pos[Y] + DEGTORAD(15);
}

uint8_t camera::Translate(V3* v_pPoint, V2* v_pResult)
{
	V3 o_Delta = *v_pPoint - i_Position;
	float f_abs_sq = o_Delta.f_Pos[0] * o_Delta.f_Pos[0] + o_Delta.f_Pos[1] * o_Delta.f_Pos[1] + o_Delta.f_Pos[2] * o_Delta.f_Pos[2];

	if (f_abs_sq > f_Frustum[Y]* f_Frustum[Y] || f_abs_sq < f_Frustum[X] * f_Frustum[X]) return GD_OUTOFBOUND;

	o_Delta.CamRotateThisOpt(i_Rotation);

	if (o_Delta.f_Pos[Z] < 0) return GD_OUTOFBOUND;

	float f_invabs = 1.0f / o_Delta.f_Pos[Z];
	float f_leftright = o_Delta.f_Pos[X] * f_invabs;
	float f_updown = o_Delta.f_Pos[Y] * f_invabs;
	...
	v_pResult->f_Pos[X] = f_leftright * f_standoff;
	v_pResult->f_Pos[Y] = f_updown * f_standoff;
	v_pResult->f_Pos[Y] *= -1; // Flip because y=0 is on top
	v_pResult->f_Pos[X] += i_Dimensions[0] / 2.0f;
	v_pResult->f_Pos[Y] += i_Dimensions[1] / 2.0f;
